...
public class ChangePasswordController {
    ...
    responseBody.put("message", "All fields mandatory");
    ...
    @PostMapping("/final")
    public ResponseEntity<Map<String, Object>> changePassword(@RequestParam(required = false) String emailorphone, @RequestParam String newPassword) {
        Optional<User> user = Optional.empty();
        if (emailorphone != null && !emailorphone.isEmpty()||(newPassword != null && !newPassword.isEmpty())) {
            user = userRepository.findByEmailOrPhone(emailorphone, emailorphone);
        }
        else
        {
        ...
        if (user.isPresent()) {
            ...
            if (lastThreePasswords != null && lastThreePasswords.stream().anyMatch(p -> passwordEncoder.matches(newPassword, p))) {
                Map<String, Object> responseBody = new HashMap<>();
                responseBody.put("success", false);
